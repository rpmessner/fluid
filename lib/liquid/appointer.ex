defmodule Liquid.Appointer do
  ...
  @doc "Verify matches between Variable and filters, data types and parts"
  ...
  def parse_name(name) do
    value = cond do
      Map.has_key?(literals(), name)            -> Map.get(literals(), name)
      Regex.match?(integer(), name)             -> String.to_integer(name)
      Regex.match?(float(), name)               -> String.to_float(name)
      Regex.match?(start_quoted_string(), name) -> Regex.replace(Liquid.quote_matcher(), name, "")
      true                                      -> Liquid.variable_parser() |> Regex.scan(name) |> List.flatten()
    end
