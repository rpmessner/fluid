    ...
    full_path = cond do
      Regex.match?(~r/\//, template_path) ->
        root |> Path.join(template_path |> Path.dirname)
             |> Path.join("_#{template_path |> Path.basename}.liquiscent")
      true ->
        root |> Path.join("_#{template_path}.liquiscent")
    end
    cond do
...
defmodule Liquescent.FileSystem do
  use GenServer
