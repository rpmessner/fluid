defmodule Liquescent.Variable do
  ...
  def integer, do: ~r/^(-?\d+)$/
  def float, do: ~r/^(-?\d[\d\.]+)$/
  def quoted_string, do: ~r/#{Liquescent.quoted_string}/
  ...
  def create(<<markup::binary>>) do
    ...
    cond do
      ...
      quoted_string |> Regex.match?(name) ->
        unquoted_name = Liquescent.quote_matcher |> Regex.replace(name, "")
        %{ variable | literal: unquoted_name }
      true ->
        [name|_] = String.split(name, " ")
        parts = Regex.scan(Liquescent.variable_parser, name) |> List.flatten
