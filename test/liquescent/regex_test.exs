...
defmodule Liquescent.RegexTest do
  use ExUnit.Case
  ...
  test :tags do
    ...
    assert_quoted_fragment ["<tr></tr>"], "<tr></tr>"
    assert_quoted_fragment ["<style", "class=\"hello\">", "</style>"], "<style class=\"hello\">' </style>"
  end
  ...
  def assert_quoted_fragment(expected, markup) do
    tokens = Regex.scan(~r/#{Liquescent.quoted_fragment}/, markup) |> List.flatten
    assert expected == tokens
  end

  def assert_variable(expected, markup) do
    tokens = Regex.scan(Liquescent.variable_parser, markup) |> List.flatten
    assert expected == tokens
  end
end
